...
import { mixedValue } from "./panel-model";
...
function NodeDesignPanel({
  ...
  onEditNodeStyle,
  onEditNodePosition,
}: Pick<PropertiesPanelProps, "onEditNodeStyle" | "onEditNodePosition"> & { entries: OverlayBridgeTargetState[] }) {
  ...
  const commitPosition = (axis: "x" | "y", raw: string) => {
    ...
    entries.forEach((entry) => {
      const current = entry.target.bounds[axis];
  ...
  return (
    <div className="properties-scroll">
      ...
      <PropertySection title="Position & size" icon={<BoxSelect size={13} />}>
        <div className="property-grid"><PropertyField label="X" value={position.x} type="number" suffix="px" testId="property-node-x" onCommit={(value) => commitPosition("x", value)} /><PropertyField label="Y" value={position.y} type="number" suffix="px" testId="property-node-y" onCommit={(value) => commitPosition("y", value)} /><PropertyField label="W" value={width} type="text" suffix="px" testId="property-node-width" onCommit={(value) => onEditNodeStyle("width", value)} /><PropertyField label="H" value={height} type="text" suffix="px" testId="property-node-height" onCommit={(value) => onEditNodeStyle("height", value)} /></div>
      </PropertySection>
      <PropertySection title="Layout" icon={<AlignCenter size={13} />}>
        <div className="property-grid property-grid-single"><PropertyField label="Display" value={styleValue(entries, "display")} onCommit={(value) => onEditNodeStyle("display", value)} /><PropertyField label="Direction" value={styleValue(entries, "flex-direction")} onCommit={(value) => onEditNodeStyle("flex-direction", value)} /><PropertyField label="Align" value={styleValue(entries, "align-items")} onCommit={(value) => onEditNodeStyle("align-items", value)} /><PropertyField label="Justify" value={styleValue(entries, "justify-content")} onCommit={(value) => onEditNodeStyle("justify-content", value)} /><PropertyField label="Gap" value={styleValue(entries, "gap")} onCommit={(value) => onEditNodeStyle("gap", value)} /></div>
      </PropertySection>
      <PropertySection title="Typography" icon={<Type size={13} />}>
        <div className="property-grid property-grid-single"><PropertyField label="Family" value={styleValue(entries, "font-family")} onCommit={(value) => onEditNodeStyle("font-family", value)} /><div className="property-grid"><PropertyField label="Size" value={styleValue(entries, "font-size")} onCommit={(value) => onEditNodeStyle("font-size", value)} /><PropertyField label="Weight" value={styleValue(entries, "font-weight")} onCommit={(value) => onEditNodeStyle("font-weight", value)} /></div><PropertyField label="Line height" value={styleValue(entries, "line-height")} onCommit={(value) => onEditNodeStyle("line-height", value)} /><div className="property-grid"><PropertyField label="Color" value={styleValue(entries, "color")} onCommit={(value) => onEditNodeStyle("color", value)} /><PropertyField label="Align" value={styleValue(entries, "text-align")} onCommit={(value) => onEditNodeStyle("text-align", value)} /></div></div>
      </PropertySection>
      <PropertySection title="Fill & border" icon={<Palette size={13} />}>
        <div className="property-grid property-grid-single"><PropertyField label="Background" value={styleValue(entries, "background-color") ?? styleValue(entries, "background")} onCommit={(value) => onEditNodeStyle("background-color", value)} /><div className="property-grid"><PropertyField label="Border" value={styleValue(entries, "border-color")} onCommit={(value) => onEditNodeStyle("border-color", value)} /><PropertyField label="Width" value={styleValue(entries, "border-width")} onCommit={(value) => onEditNodeStyle("border-width", value)} /></div><PropertyField label="Radius" value={styleValue(entries, "border-radius")} onCommit={(value) => onEditNodeStyle("border-radius", value)} /></div>
      </PropertySection>
...
function FrameDesignPanel({ frame, selection, onUpdateFrame, onMoveFrame }: Pick<PropertiesPanelProps, "selection" | "onUpdateFrame" | "onMoveFrame"> & { frame: FrameEntity }) {
  ...
  return <div className="properties-scroll"><div className="selection-summary"><span className="selection-summary-mark"><BoxSelect size={15} /></span><span><strong>{multiple ? `${selection.frameIds.length} frames` : frame.name}</strong><small>{multiple ? "Mixed selection" : "Responsive frame"}</small></span></div><PropertySection title="Position & size" icon={<BoxSelect size={13} />}><div className="property-grid"><PropertyField label="X" value={multiple ? "mixed" : formatNumber(frame.x)} type="number" suffix="px" onCommit={(value) => { const next = numericValue(value); if (next !== null) onMoveFrame(frame.id, { x: next, y: frame.y }); }} /><PropertyField label="Y" value={multiple ? "mixed" : formatNumber(frame.y)} type="number" suffix="px" onCommit={(value) => { const next = numericValue(value); if (next !== null) onMoveFrame(frame.id, { x: frame.x, y: next }); }} /><PropertyField label="W" value={multiple ? "mixed" : formatNumber(frame.width)} type="number" suffix="px" testId="property-frame-width" onCommit={(value) => { const next = numericValue(value); if (next !== null) onUpdateFrame(frame.id, { width: Math.max(24, next) }); }} /><PropertyField label="H" value={multiple ? "mixed" : formatNumber(frame.height)} type="number" suffix="px" onCommit={(value) => { const next = numericValue(value); if (next !== null) onUpdateFrame(frame.id, { height: Math.max(24, next) }); }} /></div></PropertySection><PropertySection title="Fill" icon={<Palette size={13} />}><PropertyField label="Background" value={multiple ? "mixed" : frame.background} onCommit={(value) => onUpdateFrame(frame.id, { background: value })} /></PropertySection><PropertySection title="Layout" icon={<AlignCenter size={13} />}><div className="context-note"><Gauge size={14} /><span>Frame layout constraints will appear once this frame has a layout context.</span></div></PropertySection></div>;
}

function PrototypePanel({ selection }: { selection: SelectionState }) {
  return <div className="properties-scroll"><div className="prototype-intro"><span className="prototype-icon"><Play size={16} /></span><span><strong>Prototype flows</strong><small>Connect screens and states into an experience.</small></span></div><div className="prototype-card"><div className="prototype-card-heading"><span>Interactions</span><span className="sidebar-count">{selection.frameIds.length ? "Ready" : "Empty"}</span></div><div className="prototype-empty"><Code2 size={19} /><strong>No interactions yet</strong><span>Choose a frame to define its starting point and transitions.</span></div></div><div className="prototype-card"><div className="prototype-card-heading"><span>Starting point</span><span className="prototype-status-dot" /></div><p className="context-note-copy">Prototype links, overlays, and flow settings will live here.</p></div></div>;
}
